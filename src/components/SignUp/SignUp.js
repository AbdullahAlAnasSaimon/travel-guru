...
const SignUp = () => {
  const {user, setUser, createUser, googleSignIn} = useContext(AuthContext);
  ...
  return (
    <form onSubmit={handleSignUp} className='w-4/12 mx-auto border-2 border-gray-300 p-8 my-5 rounded-lg'>
      ...
      <p className='text-red-500'>{error.slice(9, error.length - 1)}</p>
      ...
      <div>
        <button type='button' onClick={SignUpWithGoogle} className='border-2 border-gray-700 hover:bg-gray-100 w-full my-2 py-2 rounded-md'><FcGoogle className='text-xl inline-block mr-3'/> Sign Up With Google</button>
        <button type='button' className='bg-blue-800 hover:bg-blue-700 text-white w-full my-2 py-3 rounded-md'><FaFacebook className='text-xl inline-block mr-3 text-blue-100'/>Sign Up With Facebook</button>
